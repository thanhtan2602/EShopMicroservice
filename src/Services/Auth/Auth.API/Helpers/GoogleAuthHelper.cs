...
namespace Auth.API.Helpers
{
    public static class GoogleAuthHelper
    {
        ...
        public static async Task<GoogleUserInfoResponse?> GetGoogleUserInfoAsync(string accessToken)
        {
            using var client = new HttpClient();
            ...
            var json = await response.Content.ReadAsStringAsync();
            return JsonSerializer.Deserialize<GoogleUserInfoResponse>(json);
        }
    }
}
